...
% Author:   Bruno Luiz Souza Bedo <bruno.bedo@usp.br> 
    ...
    for v = 1:size(indx,2)
        ...
        % 1� step: calculating the 1st derivative for each time-series (Example: velocity data);

        series1vel=diff(series1)./(1/datafrequency); % For serie1
        series2vel=diff(series2)./(1/datafrequency); % For serie2

        %% 2� step: normalization of state variables of the phase plane
        ...
        %% 3� step: Calculation of the phase angle and relative phase
        % For series1
